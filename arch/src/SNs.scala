...
object SN8x8 extends SN(numRows=8, numCols=8, numArgIns=8, numArgOuts=3, pattern=Checkerboard) {
  override def pmuAt(x:Int, y:Int) = new PatternMemoryUnitParam(numVectorFifos=8)
  override def scalarNetwork = new ScalarNetwork() {
    // switch to switch channel width
    channelWidth("src"->"sb", "dst"->"pmu") = 8
  }
}
