...
trait Evaluation {
  ...
  def eval(op:Op3, a:Any, b:Any, c:Any)(implicit sim:Simulator):Option[AnyVal] = {
    ...
    (op,unwrap(a, op),unwrap(b, op),unwrap(c, op)) match {
      case (op@MuxOp, true     , b:Int    , _        ) => Some(b)
      case (op@MuxOp, false    , _        , c:Int    ) => Some(c)
      case (op@MuxOp, true     , b:Float  , _        ) => Some(b)
      case (op@MuxOp, false    , _        , c:Float  ) => Some(c)
      case (op@MuxOp, true     , b:Boolean, _        ) => Some(b)
      case (op@MuxOp, false    , _        , c:Boolean) => Some(c)
      case (op@MuxOp, a:Boolean, b:Int    , c:Int    ) => Some(op.eval(a, b, c))
      case (op@MuxOp, a:Boolean, b:Float  , c:Float  ) => Some(op.eval(a, b, c))
      case (op@MuxOp, a:Boolean, b:Boolean, c:Boolean) => Some(op.eval(a, b, c))
