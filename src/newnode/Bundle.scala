...
class InputEdge[B<:BundleType:ClassTag](val src:Bundle[B])(implicit design:Spade) extends prism.node.Input[SpadeNode] with DirectedEdge[OutputEdge[B]] {
  type E <: OutputEdge[B]
  def connect(p:Bundle[B]):Unit = connect(p.out) 
  def <== (p:Bundle[B]):Unit = connect(p) 
  def <== (ps:List[Bundle[B]]):Unit = ps.foreach { p => connect(p) }
}
class OutputEdge[B<:BundleType:ClassTag](val src:Bundle[B])(implicit design:Spade) extends prism.node.Output[SpadeNode] with DirectedEdge[InputEdge[B]] {
  type E <: InputEdge[B]
}
...
case class Wire[B<:BundleType:ClassTag](name:String)(implicit src:Module, design:Spade) extends Bundle[B]
abstract class Port[B<:BundleType:ClassTag](implicit src:Module, design:Spade) extends Bundle[B] {
...
case class Input[B<:BundleType:ClassTag](name:String)(implicit src:Module, design:Spade) extends Port[B] {
...
case class Output[B<:BundleType:ClassTag](name:String)(implicit src:Module, design:Spade) extends Port[B] {
